(function() {
  ...
  // Styles - refined: muted palette, inner glow cards, subtle gradients, 10px radius
  ...
  styles.textContent = [
    ...
    '#docbot-fab{position:fixed;' + posCSS + ':24px;bottom:24px;width:56px;height:56px;border-radius:16px;background:linear-gradient(135deg,' + primaryColor + ',hsl(250,35%,52%));color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 3px hsla(230,52%,52%,0.2),0 8px 24px hsla(230,52%,52%,0.2);z-index:999998;transition:all 0.3s cubic-bezier(0.4,0,0.2,1)}',
    ...
    '#docbot-header{background:linear-gradient(135deg,' + primaryColor + ',hsl(250,35%,52%));color:#fff;padding:18px 20px;flex-shrink:0;position:relative}',
    ...
    '.docbot-msg.assistant .docbot-msg-avatar{background:linear-gradient(135deg,hsl(230,30%,96%),hsl(250,20%,95%));color:hsl(230,52%,50%)}',
    '.docbot-msg-bubble{padding:10px 14px;border-radius:12px;font-size:13px;line-height:1.6;white-space:pre-wrap;word-break:break-word}',
    ...
    '.docbot-msg.user .docbot-msg-bubble{background:linear-gradient(135deg,' + primaryColor + ',hsl(250,35%,52%));color:#fff;border-bottom-right-radius:4px;box-shadow:0 1px 3px hsla(230,52%,52%,0.2)}',
    ...
    '#docbot-input:focus{border-color:hsl(230,52%,52%,0.4);box-shadow:0 0 0 3px hsl(230,52%,52%,0.08);background:#fff}',
    '#docbot-input::placeholder{color:hsl(220,8%,60%)}',
    '#docbot-send{background:linear-gradient(135deg,' + primaryColor + ',hsl(250,35%,52%));color:#fff;border:none;border-radius:10px;width:38px;height:38px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all 0.2s;box-shadow:0 1px 3px hsla(230,52%,52%,0.2)}',
  ...
  // Add welcome message
  addMessage('assistant', 'Hello! How can I help you today? Ask me anything about our documentation.');
  ...
  function sendMessage() {
    ...
    typingEl.innerHTML = '<div class="docbot-msg-avatar">AI</div><div class="docbot-typing"><span></span><span></span><span></span></div>';
    ...
    .catch(function() {
      messagesEl.removeChild(typingEl);
      addMessage('assistant', 'Sorry, I could not connect. Please try again later.');
    })
    .finally(function() {
      sending = false;
  ...
  function addMessage(role, text) {
    ...
    if (role === 'assistant') {
      div.innerHTML = '<div class="docbot-msg-avatar">AI</div><div class="docbot-msg-bubble">' + escapeHtml(text) + '</div>';
    } else {
